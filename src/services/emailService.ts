...
// Emails are sent FROM user's email TO your business email
...
export const sendEmail = async (emailData: EmailData) => {
  try {
    const response = await fetch(`${MAIL_SERVICE_URL}/api/send-email`, {
      method: 'POST',
      headers: {
...
// Send booking email - FROM user's email TO your business email
export const sendBookingEmail = async (formData: BookingFormData) => {
  try {
    // Send to business email FROM user's email
    const businessEmail = await sendEmail({
      from: formData.email, // FROM user's email
    ...
    // Send confirmation to customer FROM your business email
    const customerEmail = await sendEmail({
      from: 'info@saibaitour.mn', // FROM your business email
...
// Send contact email - FROM user's email TO your business email
