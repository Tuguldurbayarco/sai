...
export async function POST(req: NextRequest){
    try {
        ...
        const businessEmailResponse = await fetch('https://saibaitour.mn/mail-service/api/send-email', {
            ...
            body: JSON.stringify({
                from: body.email, // FROM user's email
        ...
        if (!businessEmailResponse.ok) {
            console.error('Failed to send business email:', businessEmailResponse.status);
        } else {
        ...
        return NextResponse.json({ success: true, message: 'Booking form submitted successfully' });
    } catch (error) {
        console.error('Error processing booking form:', error);
        return NextResponse.json({ success: false, error: 'Failed to process booking form' }, { status: 500 });
    }
}
